...
@ti.kernel
def solveEdge():
    grad1 = ti.Vector([0.0,0.0,0.0])
    grad2 = ti.Vector([0.0,0.0,0.0])
    for i in edge:
        pos1 = edge[i][0]
        pos2 = edge[i][1]
        dir = pos[pos1]-pos[pos2]
        grad1 = dir / dir.norm()
        grad2 = -dir / dir.norm()
        alpha = edgeCompliance /dt/dt
        C = dir.norm() - restLen[i]
        s = C / (invMass[pos1]+invMass[pos2] + alpha)

        dp1 = -s*invMass[pos1]*grad1
        dp2 = -s*invMass[pos2]*grad2
